...
#  name        = "bankingeventsourced-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application bankingeventsourced created from terraform"
  vpc_id      = "${aws-vpc}"
...
resource "aws_security_group" "mysql" {
  description = "security group for bankingeventsourced ${dEngine} created from terraform"
  vpc_id      = "${aws-vpc}"
...
#  identifier             = "bankingeventsourced-rds" # Terraform will create a unique id if not assigned
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "bankingeventsourced instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/banking-on-go-lang:latest",
      "sudo docker run -it -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/bankingeventsourced theharbormaster/banking-on-go-lang:latest"
    ]
  }
}
